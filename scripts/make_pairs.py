...
def get_homs(classes: dict, samp: int) -> list:
    """Returns a list of pairs of proteins within each class along with their homology indicator,
    1 for homologous and 0 for non-homologous.
    ...
    Returns:
        list: List of tuples where each tuple is a pair of proteins and homology indicator.
    """
...
def write_pairs(pairs: list, filename: str):
    ...
    with open(filename, 'w', encoding='utf8') as pfile:
        for pair in pairs:
            pfile.write(f'{pair[0]} {pair[1]} {pair[2]} \n')
